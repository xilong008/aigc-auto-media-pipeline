...
def generate_content_payload(domain: str = "", persona: str = "", topic_input: str = "", logger_cb=print):
    ...
    【爆款文案底层SOP（必须严格执行）】
    1. 用事实替代形容词：拒绝“假大空”和说教，必须使用具体的数字（如3万块、40%）、真实的惨痛案例、极其细节的实操坑点来作为核心论据佐证你的观点。
    2. 遵循 AIDA 结构：
       - Attention (抓眼球)：开篇直接甩出一个高频痛点场景或颠覆常识的结论，制造焦虑，决不写废话问候。
       - Interest (引兴趣)：抛出具体的量化数据或行业内幕专用词，建立专家权威感。
       - Desire (强渴望)：给出极具落地性的、拿来就能用的具体操作步骤（而不是宏观的大方向）。
       - Action (促行动)：自然真诚的私域引流钩子，引导评论区互动。
    3. 语气要像一个有血有肉的人，可以使用“别不信”、“听我一句劝”、“我见过最惨的”等拉近距离的表达。
    ...
    - "body": (必须是一篇800~1000字的深度长文。要求：严格按照上面要求的 AIDA 结构展开。段落必须包含具体的数字论据、真实惨案和行业硬核黑话。排版要清爽，多用 Emoji，短句为主。最后必须设置高转化的私域引流钩子（Call to Action）并带上3-5个相关话题标签#)
